...
class ProgressiveGan():
    ...
    def build_base_generator(self):
        ## Latent Vector
        inp = tf.keras.layers.Input(shape = (self.latent_dim,), name = "Generator Input")
        ## Reshaping the vector to image
        out = Dense(4 * 4 * RES_TO_FILTER_SIZE_GEN[4][0], gain = 1./8)(inp)
        out = tf.keras.layers.Reshape((4, 4, RES_TO_FILTER_SIZE_GEN[4][0]))(out)
        out = PixelNormalization()(out)
        ## Base Block
        out = Conv2D(RES_TO_FILTER_SIZE_GEN[4][0], 4)(out)
        out = tf.keras.layers.LeakyReLU(self.leaky_relu_alpha)(out)
        out = PixelNormalization()(out)

        out = Conv2D(RES_TO_FILTER_SIZE_GEN[4][1], 3)(out)
        out = tf.keras.layers.LeakyReLU(self.leaky_relu_alpha)(out)
        out = PixelNormalization()(out)
        
        ## Generating the base block
        return tf.keras.models.Model(inp, out, name = "Generator_4_Block")

    def build_generator_block(self, step):
        ## step parameter must be bigger than 0
        if step < 1: raise ValueError("step must be bigger than 0")

        old_img_res = STEP_TO_IMAGE_RES[step-1]
        img_res = STEP_TO_IMAGE_RES[step]
        ## Input is the last blocks output
        inp = tf.keras.layers.Input(shape = (old_img_res, old_img_res, RES_TO_FILTER_SIZE_GEN[old_img_res][1]), name = f"{img_res}_{img_res}_Generator_Block_Input")
        ## Upsampling the input 
        out = tf.keras.layers.UpSampling2D()(inp)
        ## New Block
        out = Conv2D(RES_TO_FILTER_SIZE_GEN[img_res][0], 3, name = f"{img_res}_Block_Conv_1")(out)
        out = tf.keras.layers.LeakyReLU(self.leaky_relu_alpha)(out)
        out = PixelNormalization()(out)
        

        out = Conv2D(RES_TO_FILTER_SIZE_GEN[img_res][1], 3, name = f"{img_res}_Block_Conv_2")(out)
        out = tf.keras.layers.LeakyReLU(self.leaky_relu_alpha)(out)
        out = PixelNormalization()(out)
        
        return tf.keras.models.Model(inp, out, name = f"Generator_{img_res}_Block")
    ...
    def build_standard_generator_model(self, step):
        img_res = STEP_TO_IMAGE_RES[step]
        inp = tf.keras.layers.Input(shape = (self.latent_dim, 1), name = f"{img_res}_Standard_Generator_Model_Input")
        out = self.generator_blocks[0](inp)
        for i in range(1, step+1):
            out = self.generator_blocks[i](out)
        out = self.to_rgb_blocks[step](out)
        return tf.keras.models.Model(inp, out, name = f"{img_res}_Standard_Generator_Model")
    ...
    def build_fade_in_generator_model(self, step):
        ...
        img_res = STEP_TO_IMAGE_RES[step]
        inp = tf.keras.layers.Input(shape = (self.latent_dim, 1), name = f"{img_res}_Fade_In_Generator_Model_Input")
        alpha = tf.keras.layers.Input(shape = (1,), name= "Alpha")
        old_out = self.generator_blocks[0](inp)
        for i in range(1, step):
            old_out = self.generator_blocks[i](old_out)

        new_out = self.generator_blocks[step](old_out)

        old_out = self.to_rgb_blocks[step-1](old_out)
        old_out = tf.keras.layers.UpSampling2D()(old_out)

        new_out = self.to_rgb_blocks[step](new_out)
        out = FadeIn()(old_out, new_out, alpha)
        return tf.keras.models.Model([inp, alpha], out, name = f"{img_res}_Fade_In_Generator_Model")
